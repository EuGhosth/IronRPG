...
ACHIEVEMENTS_DATA = [
    {"id": "lvl5", "name": "O Despertar", "desc": "Nível 5", "type": "level", "value": 5, "reward_title": "Novato", "xp": 500, "gold": 200},
    {"id": "rich", "name": "Patrocinado", "desc": "2000 Gold", "type": "gold", "value": 2000, "reward_title": "Patrocinado", "xp": 200, "gold": 0},
]
...
class DatabaseManager:
    ...
    def check_training_limit(self, user_id, is_strength):
        ...
        if is_strength:
            if hero['last_strength'] == today: return False, "Você já treinou Força hoje! Descanse."
            return True, "OK"
        else:
            if hero['last_cardio'] != today:
                self.cursor.execute("UPDATE heroes SET cardio_count_today=0, last_cardio_date=? WHERE user_id=?", (today, user_id)); self.conn.commit()
                return True, "OK"
            if hero['cardio_count'] >= 2: return False, "Limite de 2 Cardios diários atingido!"
            return True, "OK"

    def update_training_limit(self, user_id, is_strength):
        today = str(datetime.now().date())
        if is_strength: self.cursor.execute("UPDATE heroes SET last_strength_date=? WHERE user_id=?", (today, user_id))
        else: self.cursor.execute("UPDATE heroes SET last_cardio_date=?, cardio_count_today=cardio_count_today+1 WHERE user_id=?", (today, user_id))
        self.conn.commit()
    ...
    def check_and_unlock_achievements(self, user_id):
        hero = self.carregar_heroi(user_id)
        total_w, _ = self.get_lifetime_stats(user_id)
        unlocked_now = []
        for ach in ACHIEVEMENTS_DATA:
            ...
            if is_met:
                ...
                if not self.cursor.fetchone():
                    ...
                    hero['xp'] += ach['xp']; hero['gold'] += ach['gold']; unlocked_now.append(ach)
        if unlocked_now: self.salvar_progresso(user_id, hero)
        self.conn.commit()
        return unlocked_now
...
def main(page: ft.Page):
    ...
    def view_game():
        ...
        def render_workout_view():
            ...
            logs_display = ft.Column(spacing=2)
            if active_workout.get("session_logs"):
                logs_display.controls.append(ft.Text("Histórico Hoje:", size=12, color="grey"))
                for log in active_workout["session_logs"]: 
                    logs_display.controls.append(ft.Text(f"✅ {log}", size=12, color="green"))

            for ex in active_workout["exercises"]:
                # --- CRIAÇÃO DOS CAMPOS (Escondidos inicialmente) ---
                is_cardio = (x_target := ex['target']) == 'stamina' # Pequeno atalho
                lbl1 = "Tempo (min)" if is_cardio else "Carga (kg)"
                lbl2 = "Dist (km)" if is_cardio else "Reps"

                # Campos simples, sem bloqueios de teclado
                t_load = ft.TextField(label=lbl1, width=100, height=40, text_size=12, content_padding=10)
                t_reps = ft.TextField(label=lbl2, width=100, height=40, text_size=12, content_padding=10)
                
                # Container que guarda os campos (Começa invisível)
                input_area = ft.Container(
                    visible=False, # O SEGREDO ESTÁ AQUI
                    bgcolor="#222",
                    padding=10,
                    border_radius=5,
                    content=ft.Column([
                        ...
                        ft.ElevatedButton("SALVAR", bgcolor="green", color="white", width=200, height=30, 
                                          ...
                                          # Hackzinho para passar a referencia do container
                    ], horizontal_alignment="center")
                )
                
                # Função para MOSTRAR/ESCONDER os campos quando clica no +
                ...
                def open_hist(e, xid=ex['id'], xname=ex['name']):
                    ...
                    page.dialog = ft.AlertDialog(title=ft.Text(xname), content=rows); page.dialog.open=True; page.update()

                # Adiciona na tela: O botão + agora só revela a área de baixo
                ex_list_ui.controls.append(ft.Column([
                    ...
                    input_area # A área escondida fica aqui
                ], spacing=0))
            ...
            return ft.Container(content=ft.Column([
                ...
                rpg_container(logs_display, padding=10),
                ft.Divider(),
                ex_list_ui,
